...
private:
    // FIXME boost::intrusive cannot handle deallocated nodes,
    // not suitable for VSlab. Replace it with a custom intrusive list.
    //
    // Allocated on the heap as a temporal hack.
    boost::intrusive::list<Node> &free_nodes;
...
public:
    ...
    VSlab (Size prealloc = 1024)
	  // FIXME temporal hack
	: free_nodes (*new boost::intrusive::list<Node>),
